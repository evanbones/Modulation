...
- Added a Dirt Paths Under Blocks bugfix tweak (MC-84731), ported from Path under Fence Gates by Steveplays28.
    - Dirt paths now survive under blocks like fence gates, trapdoors, top slabs, glass panes, wall signs, and bells,
      and a shovel can flatten dirt under those blocks.
    - Wall torches can be attached to dirt path blocks.
    - Automatically disabled if Path under Fence Gates is installed.
- Added a Fix Shield Sounds bugfix tweak (MC-255756).

### Fixed

- Fixed Slot Highlight Behind Item removing the slot highlight entirely from Trinkets slots.

## [3.4.1] - 2026-09-06

### Changed

- Adjusted Debugify conditional loading (turns out the 1.21 version is pretty far behind the 26.x versions).

### Fixed

- Fixed issues with Reliable EMI (ironic).
...
- Added Dyed Flames integration for fire tweaks.

### Fixed

- Possibly fixed issues with Sunbathing Godrays???? Again????

## [3.3.1] - 2026-09-04

### Added

- Added Slot Highlight Behind Item tweak to draw most of the slot hover highlight underneath the item instead of on top
  of it.
    - Backported from 1.21.2, using the same `slot_highlight_back` and `slot_highlight_front`
      sprites under the vanilla paths, so existing resource packs should work as expected.
...
- Added Less Annoying Fire tweak to lower fire screen overlay and hide it with Fire Resistance or fire immunity.
- Added Pixel Consistent Block Drops tweak to render dropped 3D block items with Survival Test-style pixel scale.
- Added Dispenser Shears Pumpkins tweak to allow dispensers with shears to carve pumpkins.
- Added Water Bottles On Concrete Powder tweak to solidify concrete powder into concrete using water bottles or
  dispensers.
- Added Allow Mobs to Cross Rails tweak to fix MC-33359 where mobs refuse to pathfind across railway tracks.
...
- Axe stripping is now properly disabled for all modded woodtypes when enabled.

## [3.0.0] - 2026-08-19

### Added

- Added a ton of tweaks adapted from Forgery, and some more unique ones!
    - Added Better Pause Freezing tweak to freeze animated textures when paused in singleplayer.
    - Added Colored Crack Particles tweak to make item breaking particles inherit dye tints.
    - Added Ghast Charging Animation tweak restoring the classic charging animation.
    - Added Legible Signs tweak to adjust sign text contrast against wood.
    - Added Campfires Set Entities On Fire tweak.
    - Added Disable Axe Stripping tweak.
    - Added Disable Copper Scraping tweak.
    - Added No Dinnerlava tweak to disable single-block lava pockets in the Nether.
    - Added Ender Pearl Sound tweak to play teleport sounds on impact.
    - Added Spawn Egg Cracking particles and sound tweak.
    - Added Disable Anvil Damage tweak to prevent anvils from taking damage or breaking.
    - Added Loyalty tridents returning from the void).
    - Added Cactus Punching Hurts tweak.
    - Added Chaining Creepers tweak.
    - Added Furnace Minecart Accepts Any Fuel tweak.
    - Added InfiBows / Infinity bows require no arrows tweak.
    - Added Tridents Accept Sharpness enchantment tweak.
    - Added Bedrock-like Impaling damage on wet entities tweak.
    - Added Nether Cauldron water evaporation tweak.
    - Added Fire Aspect behaves as flint & steel tweak.
    - Added Cannot Open Chest When Targeted tweak.
    - Added Chat Markdown formatting support in chat messages.
    - Added Cursor Feedback tweak to change the mouse cursor shape over buttons, sliders, text fields, scroll bars,
      disabled widgets, and clickable text links.
        - Also supports EMI widgets/text fields.

## [2.2.5] - 2026-08-17

### Added

- Added an option to make passable foliage act like scaffolding.

### Removed

- Removed the density memoization fix (moved to Lithostitched).
- Removed the anvil module (moved to Experience Obliterator).
